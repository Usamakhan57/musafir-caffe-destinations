...
export function PaginationControls({ filters, totalPages }: PaginationControlsProps) {
  ...
  return (
    <nav aria-label="Pagination" className="mt-12 flex items-center justify-center gap-2">
      <Link
        href={`${ROUTES.destinations}${buildDestinationsQuery(filters, { page: Math.max(1, filters.page - 1) })}`}
      ...
      {pages.map((pageNum, idx) => {
        ...
        return (
          <span key={pageNum} className="flex items-center gap-2">
            ...
            <Link
              href={`${ROUTES.destinations}${buildDestinationsQuery(filters, { page: pageNum })}`}
              aria-current={pageNum === filters.page ? "page" : undefined}
              className={`flex h-10 w-10 items-center justify-center rounded-full text-sm font-medium transition-colors ${
                pageNum === filters.page
                  ? "bg-forest-600 text-cream-50"
                  : "text-coffee-700 hover:bg-cream-200/60"
              }`}
            >
              {pageNum}
            </Link>
      ...
      <Link
        href={`${ROUTES.destinations}${buildDestinationsQuery(filters, { page: Math.min(totalPages, filters.page + 1) })}`}
